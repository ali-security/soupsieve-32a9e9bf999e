...
class CSSParser:
    ...
    def increment_count(self, increment: int = 1) -> None:
        """Increment the selector count and check that it is still within the limit."""
    ...
    def parse_combinator(
        ...
        if not has_selector:
            ...
            # If we are in a forgiving pseudo class, drop the empty slot, but still
            # account for it so that a long run of empty slots cannot bypass the limit.
            if combinator == COMMA_COMBINATOR:
